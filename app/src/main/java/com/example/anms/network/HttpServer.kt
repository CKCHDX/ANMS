...
class HttpServer(val context: Context, private val port: Int = 8080, private val wsServer: WebSocketServer? = null, private val smsCache: SmsCache? = null) {
    ...
    private fun handleDebug(): String {
        return try {
            ...
            val json = """{"status":"ready","total_conversations":${stats["total_conversations"]},"total_messages":${stats["total_messages"]}}"""
    ...
    private fun handleGetChat(phone: String): String {
        return try {
            ...
            smsCache.loadConversation(phone)
...
function selectContact(p) {
    ...
    loadChat(p).then(() => {
        ...
        updateStatus('\u2717 Error loading', '#f44336');
    });
}
